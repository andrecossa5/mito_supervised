"""
Visualization of a single-clone with high-prevalence and low performance.
"""
...
high_p_bad_clones = (
    clones
    .groupby(['sample', 'comparison'])
    .apply(lambda x: x['clone_prevalence'].unique()[0]>t_high and not (x['AUCPR']>aucpr_t).any() )
    .to_frame('to_retain').reset_index()
    .loc[lambda x: x['to_retain']]
)
high_p_bad_clones['comparison'].unique()[0]

# MDA_clones, CCGCAATAAATGCGACTT_vs_rest here
sample = 'MDA_PT'
clone = 'TTCGCGAACCTGATACAT'
clone_d = (
    clones
    .query('comparison == "TTCGCGAACCTGATACAT_vs_rest"')
    .sort_values('AUCPR', ascending=False)
    .head(1)
    .T.iloc[:,0]
    .to_dict()
...
with open(path_pickle, 'rb') as f:
    d = pickle.load(f)
variants = d['trained_models']['TTCGCGAACCTGATACAT_vs_rest']['variants']
...
ax.text(.5, .15, f'n cells: {clone_d["ncells_clone"]} (sample: {clone_d["ncells_sample"]})', transform=ax.transAxes)
ax.text(.5, .1, f'Prevalence: {clone_d["clone_prevalence"]:.2e}', transform=ax.transAxes)
ax.text(.5, .05, f'AUCPR: {clone_d["AUCPR"]:.2f}', transform=ax.transAxes)
fig.savefig(os.path.join(path_viz, 'the_figghiendrocchie.png'))
...
var_colors = create_palette(df_, 'variable', 'tab10', lightness=.5, saturation=.75)

box(df_, x='group', y='value', by='variable', c=var_colors, ax=ax)
format_ax(ax, title='Poorly exclusive MT-SNVs', ylabel='AF')
ax.spines[['right', 'top']].set_visible(False)

ax.text(.05, .88, 
    f'9637_T>C, % +clone, +rest: {vois_df.loc["9637_T>C", "perc_clone"]:.2f}, {vois_df.loc["9637_T>C", "perc_rest"]:.2f}', 
    transform=ax.transAxes, fontdict={'color':var_colors["9637_T>C"], 'fontweight': 'bold'}
)
ax.text(.05, .81, 
    f'6573_G>A, % +clone, +rest: {vois_df.loc["6573_G>A", "perc_clone"]:.2f}, {vois_df.loc["6573_G>A", "perc_rest"]:.2f}', 
    transform=ax.transAxes, fontdict={'color':var_colors["6573_G>A"], 'fontweight': 'bold'}
)
ax.text(.05, .74, 
    f'8147_C>T, % +clone, +rest: {vois_df.loc["8147_C>T", "perc_clone"]:.2f}, {vois_df.loc["8147_C>T", "perc_rest"]:.2f}', 
    transform=ax.transAxes, fontdict={'color':var_colors["8147_C>T"], 'fontweight': 'bold'}
)
...
g = cells_vars_heatmap(a, cmap='mako', cell_anno='GBC', anno_colors=colors, heat_label=None, 
    ...
    legend_ncol=1, xticks_size=5, order='diagonal_clones', vmax=0.05
)
