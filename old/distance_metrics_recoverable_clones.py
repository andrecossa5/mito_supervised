...
def main():
    ...
    # GT vars
    file = f'{sample}_GT_variants.pickle'
    with open(os.path.join(path_tmp, 'variant_subsets_and_GT', file), 'rb') as f:
        d_variants = pickle.load(f)
    variants = d_variants['(0.75,0.25)']

    # Recoverable cells
    clones = pd.read_csv(
        os.path.join(path_data, sample, 'good_clones.csv'), 
        index_col=0
    )['clone']
    cells = afm.obs['GBC'].isin(clones).loc[lambda x:x].index

    # Filter
    _, a = filter_cells_and_vars(afm, sample=sample, variants=variants, cells=cells)
    a = nans_as_zeros(a)
    labels = a.obs['GBC']
    ...
    # Save
    with open(os.path.join(path_sample, f'evaluation_metrics_aucpr_{sample}_GT.pickle'), 'wb') as f:
    ...
    fig.tight_layout()
    fig.savefig(
        os.path.join(path_viz, f'evaluation_metrics_aucpr_{sample}_GT.png'),
        dpi=500
    )
    ...
    # Save
    with open(os.path.join(path_sample, f'evaluation_metrics_kNN_{sample}_GT.pickle'), 'wb') as f:
        pickle.dump(results, f)
    ...
    plot_heatmap(df, ax=ax, label='n', title=f'{sample}, GT: shared NN (median)')
    fig.tight_layout()
    fig.savefig(
        os.path.join(
            path_viz, f'kNN_overlaps_{sample}_GT.png'
        ),
        dpi=500
    )
